define(["dojo/_base/declare", "dojo/_base/lang", "dojo/when", "dojox/widget/_Invalidating"],
	...
	return declare(_Invalidating, {
		...
		_updateItem: function(object, previousIndex, newIndex){
			...
			if(previousIndex != -1){
				...
				if(newIndex != previousIndex){
					// remove
					layoutNeeded = this.removeItem(previousIndex, newItem, items);
				}else{
					// this is a put, previous and new index identical
					layoutNeeded = this.putItem(previousIndex, newItem, items);
				}
			}else if(newIndex != -1){
				// this is a add
				layoutNeeded = this.addItem(newIndex, newItem, items);
		...
		removeItem: function(index, item, items){
			...
			// returns: Boolean
			//		Whether a complete relayout is needed or only the visual aspect has changed.  
			// tags:
			//		protected
